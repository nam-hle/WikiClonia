...
const parse = (s, l, i, e) => {
  ...
  while (i < l) {
    ...
    for (let j = 0; j < allowElements.length; j++) {
      ...
      if (taste(s, element.startToken, i)) {
        ...
        if (element.elementName == "Heading1") {
          // let heading = [];
          // console.log(cur);
          curHeading1 = cur.children[0].text;
          headings[curHeading1] = [];
          // headings.push(heading);
        }
        if (element.elementName == "Heading2") {
          // console.log(elementName, headings);
          if (headings && headings[curHeading1])
            headings[curHeading1].push(cur.children[0].text);
        }
        res.push(cur);
        break;
      }
  ...
  if (!meta) meta = {};
  if (!Object.keys(headings).length) headings = null;
  // if (headings) console.log(headings);
  return [
    i,
    children === null
      ? { elementName, ...meta, headings }
      : { elementName, children, ...meta, headings },
    plain
  ];
};
...
console.log(JSON.stringify(main("[[Bokmål]]"), null, 2));
// console.log(
//   JSON.stringify(
//     main(
