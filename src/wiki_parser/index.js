...
const parse = (s, l, i, e) => {
  ...
  let buffer = "",
    ...
    res = [];
  l = l === null ? s.length : l;
  let { name, startToken, endToken, allowElements } = e;
  ...
  if (buffer) res.push(buffer);
  res = internalParse(e, res, plain);
  return [i, { name, content: res }, plain];
};
// parse(s, (l = null), (i = 0), Global);
console.log(JSON.stringify(parse(s, (l = null), (i = 0), Global)[1]));
// console.log(
//   /^ name="([^\"]+)">([\s\S]*)$/gm.exec(
//     ` name="NatGeo 2004">{{cite web\n |title=Oldest Known Pet Cat? 9500-year-old Burial Found on Cyprus\n |url= http://news.nationalgeographic.com/news/2004/04/0408_040408_oldestpetcat.html\n |accessdate=March 6, 2007\n |date=April 8, 2004\n |work=National Geographic News\n |publisher=National Geographic Society\n}}`
//   )
// );
