  ...
  if (element.elementName == "Reference") {
    ...
    */
    ...
    if ((match = R_MEDIA.exec(first))) {
      ...
      return { ...res, options, caption, rootUrl, plain };
    }
...
const parse = (s, l, i, e) => {
  ...
  while (i < l) {
    ...
    for (let j = 0; j < allowElements.length; j++) {
      ...
      if (taste(s, element.startToken, i)) {
        if (buffer) res.push({ elementName: "Text", content: buffer });
        buffer = "";
        has = true;
        let curPlain;
  ...
  if (buffer) res.push({ elementName: "Text", content: buffer });
  res = internalParse(e, res, plain, options);
  return [i, { elementName, content: res }, plain];
};
